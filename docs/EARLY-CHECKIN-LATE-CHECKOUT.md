...
- `earliest_early_check_in_time` dan `latest_late_checkout_time` juga merupakan konfigurasi properti. Nilai produksi sengaja belum ditetapkan dan akan diisi Owner melalui pengaturan sebelum UAT/go-live.
- Bila batas belum aktif, sistem tidak boleh mengartikan permintaan customer sebagai approval tanpa batas; Front Office tetap memakai readiness, next-arrival, turnover, permission, dan policy guard.
...
- Maintenance, block, room move, maximum cutoff, serta folio/payment impact.
...
- Approved time melewati batas yang harus diproses sebagai extension tetapi inventory malam berikutnya tidak tersedia.
...
- Jika approved time melewati configured overnight threshold atau masuk malam berikutnya, permintaan harus diubah menjadi stay extension dan menjalankan inventory locking per malam.
...
- Late checkout melewati overnight threshold memakai extension workflow dan gagal atomik bila inventory tidak tersedia.
...
- Isi nilai produksi `earliest_early_check_in_time` dan `latest_late_checkout_time`; field serta governance konfigurasinya sudah disetujui.
- Minimum cleaning/inspection buffer serta threshold `next guest sudah dekat`.
- Overnight threshold yang mengubah late checkout menjadi extension.
